/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @dfinity/caffeine-bindgen@0.1.0-beta.7.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { IDL } from '@icp-sdk/core/candid';

export const RestaurantId = IDL.Text;
export const EmployeeId = IDL.Text;
export const EmployeeRole = IDL.Variant({
  'bar' : IDL.Null,
  'kitchen' : IDL.Null,
  'waiter' : IDL.Null,
});
export const AccessCode = IDL.Text;
export const Employee = IDL.Record({
  'id' : EmployeeId,
  'principal' : IDL.Opt(IDL.Principal),
  'name' : IDL.Text,
  'role' : EmployeeRole,
  'accessCode' : AccessCode,
});
export const MenuItemId = IDL.Text;
export const EmployeeCategory = IDL.Variant({
  'bar' : IDL.Null,
  'kitchen' : IDL.Null,
  'waiter' : IDL.Null,
});
export const ComponentId = IDL.Text;
export const MenuItem = IDL.Record({
  'id' : MenuItemId,
  'name' : IDL.Text,
  'routedBy' : EmployeeCategory,
  'description' : IDL.Text,
  'componentId' : ComponentId,
  'category' : IDL.Text,
  'price' : IDL.Nat,
});
export const UserRole = IDL.Variant({
  'admin' : IDL.Null,
  'user' : IDL.Null,
  'guest' : IDL.Null,
});
export const CanisterId = IDL.Text;
export const RestaurantIdentityId = IDL.Text;
export const Component = IDL.Record({
  'id' : ComponentId,
  'name' : IDL.Text,
  'description' : IDL.Text,
});
export const TableId = IDL.Text;
export const Table = IDL.Record({
  'id' : TableId,
  'customerLink' : IDL.Text,
  'active' : IDL.Bool,
  'name' : IDL.Text,
  'location' : IDL.Text,
});
export const Currency = IDL.Variant({ 'eur' : IDL.Null, 'usd' : IDL.Null });
export const ColorScheme = IDL.Record({
  'primaryColor' : IDL.Text,
  'accentColor' : IDL.Text,
  'secondaryColor' : IDL.Text,
});
export const Restaurant = IDL.Record({
  'id' : RestaurantId,
  'manager' : IDL.Principal,
  'name' : IDL.Text,
  'components' : IDL.Vec(Component),
  'description' : IDL.Text,
  'tables' : IDL.Vec(Table),
  'address' : IDL.Text,
  'defaultCurrency' : Currency,
  'identityId' : RestaurantIdentityId,
  'colorScheme' : ColorScheme,
});
export const AccessCodeData = IDL.Record({
  'expiresAt' : IDL.Opt(IDL.Int),
  'code' : AccessCode,
  'createdAt' : IDL.Int,
  'role' : UserRole,
  'isActive' : IDL.Bool,
  'restaurantId' : IDL.Opt(RestaurantId),
  'employeeId' : IDL.Opt(EmployeeId),
});
export const UserProfile = IDL.Record({ 'name' : IDL.Text });
export const OrderId = IDL.Nat;
export const OrderStatus = IDL.Variant({
  'paid' : IDL.Null,
  'ordered' : IDL.Null,
  'readyToServe' : IDL.Null,
  'toBePaid' : IDL.Null,
});
export const OrderSource = IDL.Variant({
  'bar' : IDL.Null,
  'table' : IDL.Null,
  'kitchen' : IDL.Null,
});
export const OrderItem = IDL.Record({
  'componentId' : ComponentId,
  'quantity' : IDL.Nat,
  'category' : EmployeeCategory,
  'menuItemId' : MenuItemId,
});
export const OrderBatch = IDL.Record({
  'id' : OrderId,
  'status' : OrderStatus,
  'source' : OrderSource,
  'toLabel' : IDL.Text,
  'tableId' : TableId,
  'restaurantId' : RestaurantId,
  'currency' : Currency,
  'timestamp' : IDL.Int,
  'customerId' : IDL.Opt(IDL.Principal),
  'items' : IDL.Vec(OrderItem),
});
export const ExchangeRate = IDL.Record({
  'usdToEur' : IDL.Float64,
  'eurToUsd' : IDL.Float64,
});

export const idlService = IDL.Service({
  'addEmployee' : IDL.Func([RestaurantId, Employee], [], []),
  'addMenuItem' : IDL.Func([RestaurantId, MenuItem], [], []),
  'assignCallerUserRole' : IDL.Func([IDL.Principal, UserRole], [], []),
  'assignRestaurantToCanister' : IDL.Func([RestaurantId, CanisterId], [], []),
  'checkEmployeeLink' : IDL.Func(
      [IDL.Principal, RestaurantIdentityId],
      [IDL.Opt(EmployeeId)],
      ['query'],
    ),
  'claimAccessCode' : IDL.Func([AccessCode, RestaurantIdentityId], [], []),
  'createRestaurant' : IDL.Func([Restaurant], [], []),
  'deactivateAccessCode' : IDL.Func([AccessCode], [], []),
  'deleteEmployee' : IDL.Func([RestaurantId, EmployeeId], [], []),
  'deleteMenuItem' : IDL.Func([RestaurantId, MenuItemId], [], []),
  'deleteTable' : IDL.Func([RestaurantId, TableId], [], []),
  'generateAccessCode' : IDL.Func(
      [UserRole, IDL.Opt(RestaurantId), IDL.Opt(EmployeeId)],
      [AccessCode],
      [],
    ),
  'getAccessCodesByRestaurant' : IDL.Func(
      [RestaurantId],
      [IDL.Vec(AccessCodeData)],
      ['query'],
    ),
  'getAllRestaurants' : IDL.Func([], [IDL.Vec(Restaurant)], ['query']),
  'getAvailableAccessCodes' : IDL.Func(
      [RestaurantIdentityId],
      [IDL.Vec(AccessCode)],
      ['query'],
    ),
  'getCallerUserProfile' : IDL.Func([], [IDL.Opt(UserProfile)], ['query']),
  'getCallerUserRole' : IDL.Func([], [UserRole], ['query']),
  'getCanisterForRestaurant' : IDL.Func(
      [RestaurantId],
      [IDL.Opt(CanisterId)],
      ['query'],
    ),
  'getCustomerOrderHistory' : IDL.Func([], [IDL.Vec(OrderBatch)], ['query']),
  'getEmployees' : IDL.Func([RestaurantId], [IDL.Vec(Employee)], ['query']),
  'getExchangeRate' : IDL.Func([], [ExchangeRate], ['query']),
  'getMenuItems' : IDL.Func([RestaurantId], [IDL.Vec(MenuItem)], ['query']),
  'getMenuItemsByCategory' : IDL.Func(
      [RestaurantId],
      [IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(MenuItem)))],
      ['query'],
    ),
  'getOrder' : IDL.Func([OrderId], [IDL.Opt(OrderBatch)], ['query']),
  'getOrdersBySource' : IDL.Func(
      [RestaurantId, OrderSource],
      [IDL.Vec(OrderBatch)],
      ['query'],
    ),
  'getOrdersByStatus' : IDL.Func(
      [RestaurantId, OrderStatus],
      [IDL.Vec(OrderBatch)],
      ['query'],
    ),
  'getOrdersByStatusAndSource' : IDL.Func(
      [RestaurantId, OrderStatus, OrderSource],
      [IDL.Vec(OrderBatch)],
      ['query'],
    ),
  'getOrdersForRestaurant' : IDL.Func(
      [RestaurantId],
      [IDL.Vec(OrderBatch)],
      ['query'],
    ),
  'getOrdersForWaiter' : IDL.Func(
      [RestaurantId],
      [IDL.Vec(OrderBatch)],
      ['query'],
    ),
  'getRestaurant' : IDL.Func([RestaurantId], [IDL.Opt(Restaurant)], ['query']),
  'getRestaurantByTableId' : IDL.Func(
      [TableId],
      [IDL.Opt(Restaurant)],
      ['query'],
    ),
  'getUserProfile' : IDL.Func(
      [IDL.Principal],
      [IDL.Opt(UserProfile)],
      ['query'],
    ),
  'getValidAccessCodesForRestaurant' : IDL.Func(
      [RestaurantId],
      [IDL.Vec(AccessCodeData)],
      ['query'],
    ),
  'initializeAccessControl' : IDL.Func([], [], []),
  'isCallerAdmin' : IDL.Func([], [IDL.Bool], ['query']),
  'placeOrder' : IDL.Func([OrderBatch], [OrderId], []),
  'saveCallerUserProfile' : IDL.Func([UserProfile], [], []),
  'setCustomAccessCode' : IDL.Func(
      [RestaurantId, EmployeeId, AccessCode],
      [],
      [],
    ),
  'updateAccessCode' : IDL.Func([AccessCode], [AccessCode], []),
  'updateExchangeRate' : IDL.Func([ExchangeRate], [], []),
  'updateOrderStatus' : IDL.Func([OrderId, OrderStatus], [], []),
  'updateRestaurant' : IDL.Func([Restaurant], [], []),
  'validateRestaurantIdentityId' : IDL.Func(
      [RestaurantIdentityId],
      [IDL.Bool],
      ['query'],
    ),
  'verifyAccessCode' : IDL.Func(
      [AccessCode],
      [
        IDL.Record({
          'role' : IDL.Opt(UserRole),
          'error' : IDL.Text,
          'restaurantId' : IDL.Opt(RestaurantId),
          'employeeId' : IDL.Opt(EmployeeId),
          'isValid' : IDL.Bool,
        }),
      ],
      ['query'],
    ),
});

export const idlInitArgs = [];

export const idlFactory = ({ IDL }) => {
  const RestaurantId = IDL.Text;
  const EmployeeId = IDL.Text;
  const EmployeeRole = IDL.Variant({
    'bar' : IDL.Null,
    'kitchen' : IDL.Null,
    'waiter' : IDL.Null,
  });
  const AccessCode = IDL.Text;
  const Employee = IDL.Record({
    'id' : EmployeeId,
    'principal' : IDL.Opt(IDL.Principal),
    'name' : IDL.Text,
    'role' : EmployeeRole,
    'accessCode' : AccessCode,
  });
  const MenuItemId = IDL.Text;
  const EmployeeCategory = IDL.Variant({
    'bar' : IDL.Null,
    'kitchen' : IDL.Null,
    'waiter' : IDL.Null,
  });
  const ComponentId = IDL.Text;
  const MenuItem = IDL.Record({
    'id' : MenuItemId,
    'name' : IDL.Text,
    'routedBy' : EmployeeCategory,
    'description' : IDL.Text,
    'componentId' : ComponentId,
    'category' : IDL.Text,
    'price' : IDL.Nat,
  });
  const UserRole = IDL.Variant({
    'admin' : IDL.Null,
    'user' : IDL.Null,
    'guest' : IDL.Null,
  });
  const CanisterId = IDL.Text;
  const RestaurantIdentityId = IDL.Text;
  const Component = IDL.Record({
    'id' : ComponentId,
    'name' : IDL.Text,
    'description' : IDL.Text,
  });
  const TableId = IDL.Text;
  const Table = IDL.Record({
    'id' : TableId,
    'customerLink' : IDL.Text,
    'active' : IDL.Bool,
    'name' : IDL.Text,
    'location' : IDL.Text,
  });
  const Currency = IDL.Variant({ 'eur' : IDL.Null, 'usd' : IDL.Null });
  const ColorScheme = IDL.Record({
    'primaryColor' : IDL.Text,
    'accentColor' : IDL.Text,
    'secondaryColor' : IDL.Text,
  });
  const Restaurant = IDL.Record({
    'id' : RestaurantId,
    'manager' : IDL.Principal,
    'name' : IDL.Text,
    'components' : IDL.Vec(Component),
    'description' : IDL.Text,
    'tables' : IDL.Vec(Table),
    'address' : IDL.Text,
    'defaultCurrency' : Currency,
    'identityId' : RestaurantIdentityId,
    'colorScheme' : ColorScheme,
  });
  const AccessCodeData = IDL.Record({
    'expiresAt' : IDL.Opt(IDL.Int),
    'code' : AccessCode,
    'createdAt' : IDL.Int,
    'role' : UserRole,
    'isActive' : IDL.Bool,
    'restaurantId' : IDL.Opt(RestaurantId),
    'employeeId' : IDL.Opt(EmployeeId),
  });
  const UserProfile = IDL.Record({ 'name' : IDL.Text });
  const OrderId = IDL.Nat;
  const OrderStatus = IDL.Variant({
    'paid' : IDL.Null,
    'ordered' : IDL.Null,
    'readyToServe' : IDL.Null,
    'toBePaid' : IDL.Null,
  });
  const OrderSource = IDL.Variant({
    'bar' : IDL.Null,
    'table' : IDL.Null,
    'kitchen' : IDL.Null,
  });
  const OrderItem = IDL.Record({
    'componentId' : ComponentId,
    'quantity' : IDL.Nat,
    'category' : EmployeeCategory,
    'menuItemId' : MenuItemId,
  });
  const OrderBatch = IDL.Record({
    'id' : OrderId,
    'status' : OrderStatus,
    'source' : OrderSource,
    'toLabel' : IDL.Text,
    'tableId' : TableId,
    'restaurantId' : RestaurantId,
    'currency' : Currency,
    'timestamp' : IDL.Int,
    'customerId' : IDL.Opt(IDL.Principal),
    'items' : IDL.Vec(OrderItem),
  });
  const ExchangeRate = IDL.Record({
    'usdToEur' : IDL.Float64,
    'eurToUsd' : IDL.Float64,
  });
  
  return IDL.Service({
    'addEmployee' : IDL.Func([RestaurantId, Employee], [], []),
    'addMenuItem' : IDL.Func([RestaurantId, MenuItem], [], []),
    'assignCallerUserRole' : IDL.Func([IDL.Principal, UserRole], [], []),
    'assignRestaurantToCanister' : IDL.Func([RestaurantId, CanisterId], [], []),
    'checkEmployeeLink' : IDL.Func(
        [IDL.Principal, RestaurantIdentityId],
        [IDL.Opt(EmployeeId)],
        ['query'],
      ),
    'claimAccessCode' : IDL.Func([AccessCode, RestaurantIdentityId], [], []),
    'createRestaurant' : IDL.Func([Restaurant], [], []),
    'deactivateAccessCode' : IDL.Func([AccessCode], [], []),
    'deleteEmployee' : IDL.Func([RestaurantId, EmployeeId], [], []),
    'deleteMenuItem' : IDL.Func([RestaurantId, MenuItemId], [], []),
    'deleteTable' : IDL.Func([RestaurantId, TableId], [], []),
    'generateAccessCode' : IDL.Func(
        [UserRole, IDL.Opt(RestaurantId), IDL.Opt(EmployeeId)],
        [AccessCode],
        [],
      ),
    'getAccessCodesByRestaurant' : IDL.Func(
        [RestaurantId],
        [IDL.Vec(AccessCodeData)],
        ['query'],
      ),
    'getAllRestaurants' : IDL.Func([], [IDL.Vec(Restaurant)], ['query']),
    'getAvailableAccessCodes' : IDL.Func(
        [RestaurantIdentityId],
        [IDL.Vec(AccessCode)],
        ['query'],
      ),
    'getCallerUserProfile' : IDL.Func([], [IDL.Opt(UserProfile)], ['query']),
    'getCallerUserRole' : IDL.Func([], [UserRole], ['query']),
    'getCanisterForRestaurant' : IDL.Func(
        [RestaurantId],
        [IDL.Opt(CanisterId)],
        ['query'],
      ),
    'getCustomerOrderHistory' : IDL.Func([], [IDL.Vec(OrderBatch)], ['query']),
    'getEmployees' : IDL.Func([RestaurantId], [IDL.Vec(Employee)], ['query']),
    'getExchangeRate' : IDL.Func([], [ExchangeRate], ['query']),
    'getMenuItems' : IDL.Func([RestaurantId], [IDL.Vec(MenuItem)], ['query']),
    'getMenuItemsByCategory' : IDL.Func(
        [RestaurantId],
        [IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(MenuItem)))],
        ['query'],
      ),
    'getOrder' : IDL.Func([OrderId], [IDL.Opt(OrderBatch)], ['query']),
    'getOrdersBySource' : IDL.Func(
        [RestaurantId, OrderSource],
        [IDL.Vec(OrderBatch)],
        ['query'],
      ),
    'getOrdersByStatus' : IDL.Func(
        [RestaurantId, OrderStatus],
        [IDL.Vec(OrderBatch)],
        ['query'],
      ),
    'getOrdersByStatusAndSource' : IDL.Func(
        [RestaurantId, OrderStatus, OrderSource],
        [IDL.Vec(OrderBatch)],
        ['query'],
      ),
    'getOrdersForRestaurant' : IDL.Func(
        [RestaurantId],
        [IDL.Vec(OrderBatch)],
        ['query'],
      ),
    'getOrdersForWaiter' : IDL.Func(
        [RestaurantId],
        [IDL.Vec(OrderBatch)],
        ['query'],
      ),
    'getRestaurant' : IDL.Func(
        [RestaurantId],
        [IDL.Opt(Restaurant)],
        ['query'],
      ),
    'getRestaurantByTableId' : IDL.Func(
        [TableId],
        [IDL.Opt(Restaurant)],
        ['query'],
      ),
    'getUserProfile' : IDL.Func(
        [IDL.Principal],
        [IDL.Opt(UserProfile)],
        ['query'],
      ),
    'getValidAccessCodesForRestaurant' : IDL.Func(
        [RestaurantId],
        [IDL.Vec(AccessCodeData)],
        ['query'],
      ),
    'initializeAccessControl' : IDL.Func([], [], []),
    'isCallerAdmin' : IDL.Func([], [IDL.Bool], ['query']),
    'placeOrder' : IDL.Func([OrderBatch], [OrderId], []),
    'saveCallerUserProfile' : IDL.Func([UserProfile], [], []),
    'setCustomAccessCode' : IDL.Func(
        [RestaurantId, EmployeeId, AccessCode],
        [],
        [],
      ),
    'updateAccessCode' : IDL.Func([AccessCode], [AccessCode], []),
    'updateExchangeRate' : IDL.Func([ExchangeRate], [], []),
    'updateOrderStatus' : IDL.Func([OrderId, OrderStatus], [], []),
    'updateRestaurant' : IDL.Func([Restaurant], [], []),
    'validateRestaurantIdentityId' : IDL.Func(
        [RestaurantIdentityId],
        [IDL.Bool],
        ['query'],
      ),
    'verifyAccessCode' : IDL.Func(
        [AccessCode],
        [
          IDL.Record({
            'role' : IDL.Opt(UserRole),
            'error' : IDL.Text,
            'restaurantId' : IDL.Opt(RestaurantId),
            'employeeId' : IDL.Opt(EmployeeId),
            'isValid' : IDL.Bool,
          }),
        ],
        ['query'],
      ),
  });
};

export const init = ({ IDL }) => { return []; };